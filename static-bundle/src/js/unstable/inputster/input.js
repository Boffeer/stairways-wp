...
const fileValid = (input, name, error) => {
    if (input.closest('form').querySelector('.file-info'))
        input.closest('form').querySelector('.file-info').remove();

    input.closest('.input-attach').insertAdjacentHTML('afterend', `
      <span class="file-info ${error ? 'file-error' : ''}">${name}</span>
    `);
};
...
document.querySelectorAll('.input-attach__field').forEach(el => {
    el.addEventListener('change', function(e) {
        ...
        if (!FORMATFILE.exec(name)) {
            fileValid(e.target, ERRORTEXT, true);
        } else {
            fileValid(e.target, name);
        }
    });
});